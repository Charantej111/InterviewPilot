...
export const interviewService = {
  ...
  async createInterview(params: CreateInterviewParams): Promise<InterviewSession> {
    ...
    const savedQuestions: Question[] = (createdQuestions || []).map((q) => ({
      ...
      expectedAnswerCharacteristics: (q.expected_signals as unknown as string[]) || ['States clear problem context and initial assumptions'],
      parentQuestionId: q.parent_question_id || null,
      category: q.category,
      text: q.question_text,
      intent: q.intent || `Assess candidate's ability in ${q.category || 'problem solving'}.`,
    ...
    rawEvals.forEach((e: any) => {
      ...
      if (correspondingAnswer) {
        feedbacksMap[correspondingAnswer.question_id] = {
          ...
          relevanceGate: { status: 'answered', score: Number(e.relevance), reason: 'Evaluated response' },
