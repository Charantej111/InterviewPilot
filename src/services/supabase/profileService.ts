  ...
  async getProfile(userId: string, authEmail?: string): Promise<{ profile: UserProfile; preferences: UserPreferences } | null> {
    ...
    if (!data) {
      ...
      return {
        profile: {
          id: inserted.id,
          name: inserted.full_name || 'Candidate',
          email: authEmail || '',
          avatarUrl: inserted.avatar_url || undefined,
          targetRole: inserted.target_role || 'Senior Product Manager',
          targetCompanies: inserted.target_companies || ['Stripe', 'Acme Corp'],
          experienceLevel: (inserted.experience_level as UserProfile['experienceLevel']) || 'Mid-level',
          streakDays: inserted.streak_days || 1,
    ...
    return {
      profile: {
        ...
        targetRole: data.target_role || 'Senior Product Manager',
        targetCompanies: data.target_companies || ['Stripe', 'Acme Corp'],
        experienceLevel: (data.experience_level as UserProfile['experienceLevel']) || 'Mid-level',
